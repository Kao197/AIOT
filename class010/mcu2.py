...
class MQTT:
    ...
    方法:
         connect(mqClient0): 連接到MQTT伺服器。
         subscribe(mqClient0, topic: str, on_message: function): 訂閱主題並設置回調函式。
         check_msg(mqClient0): 檢查是否有收到訊息並保持連線。
    """

    def __init__(self, client_id, server, user, password, keepalive, mqClient0):
    ...
    def on_message(self, topic, msg_received):
        msg = msg_received.decode("utf-8")
        topic = topic.decode("utf-8")
        print(f"my subscribe topic:{topic}, message: {msg}")

    def subscribe(self, mqClient0, topic: str, on_message: function):
        mqClient0.set_callback(on_message)  # 設定接收訊息的時候要執行的函式
        mqClient0.subscribe(topic)  # 訂閱主題

    def check_msg(self, mqClient0):
